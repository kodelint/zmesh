...
task :install => [:submodule_init, :submodules] do
  puts
  puts "======================================================"
  puts "Welcome to zMeSh Installation."
  puts "======================================================"
  puts
  ...
  if want_to_install?('nvim configuration (highly recommended)')
    install_files(Dir.glob('{init.vim}'))
...
task :submodules do
  unless ENV["SKIP_SUBMODULES"]
    puts "======================================================"
    puts "Downloading zmesh submodules...please wait"
    puts "======================================================"
...
task :install_vimplug do
  puts "======================================================"
  puts "Installing and updating vimlug."
  puts "The installer will now proceed to run PlugInstall or +PlugInstall to install plugins."
  puts "======================================================"
  ...
  unless File.exists?(vimplug_path)
    run %{
      ...
      curl -fLo ~/.zmesh/nvim/autoload/plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim
    }
  end
...
def install_fonts
  ...
  puts "======================================================"
  run %{ cp -f $HOME/.zmesh/fonts/* $HOME/Library/Fonts } if RUBY_PLATFORM.downcase.include?("darwin")
  run %{ mkdir -p ~/.fonts && cp ~/.zmesh/fonts/* ~/.fonts && fc-cache -vf ~/.fonts } if RUBY_PLATFORM.downcase.include?("linux")
  puts
end
...
def install_prezto
  ...
  run %{ ln -nfs "$HOME/.zmesh/zsh/prezto" "${ZDOTDIR:-$HOME}/.zprezto" }

  # The prezto runcoms are only going to be installed if zprezto has never been installed
  install_files(Dir.glob('zsh/prezto/runcoms/z*'), :symlink)

  puts
  puts "Overriding prezto ~/.zpreztorc with zmesh's zpreztorc to enable additional modules..."
  run %{ ln -nfs "$HOME/.zmesh/zsh/prezto-override/zpreztorc" "${ZDOTDIR:-$HOME}/.zpreztorc" }
